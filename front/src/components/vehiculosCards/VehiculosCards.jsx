...
import { getVehiculos0km } from "../../store/actions/vehiculosActions";
import foto from "../../img/feat.webp";
import { Link } from "react-router-dom"; // Importa Link de React Router

const VehiculosCards = ({ vehiculos0km, getVehiculos0km }) => {
  useEffect(() => {
    getVehiculos0km();
  }, [getVehiculos0km]);

console.log('yooo',vehiculos0km)

  return (
    <div>
      {/* <Sidebar /> */}
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 bg-sky-100">
        {vehiculos0km.slice(0,8).map((vehiculo) => {
          return (
            <div
              key={vehiculo.id}
...
const mapStateToProps = (state) => {
  return {
    vehiculos0km: state.vehiculos0km,
  };
};

const mapDispatchToProps = (dispatch) => {
  return {
    getVehiculos0km: () => dispatch(getVehiculos0km()),
  };
};
